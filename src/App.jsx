...
const mediaItems = [
  ...
  { type: 'video', category: 'food', label: 'DGF Movement Highlights', title: 'DGF Food Distribution & Humanitarian Field Movement', description: 'Watch our volunteers in action distributing food relief packages to families in Gowon Estate, Lagos.' },
  { type: 'video', category: 'medical', label: 'Health Screening', title: 'Free Health & Medical Care Outreach Video', description: 'Full video coverage of healthcare consultations, basic medicines, and health checks for community members.' },
  { type: 'video', category: 'all', label: 'Community Voice', title: 'Dreyson Global Foundation 2024 Impact Story', description: 'Heartfelt testimonials and video highlights from community members receiving support from DGF.' },
];
...
function Home({ onDonate }) {
  ...
  return <>
    ...
    <section className="stats-section"><div className="stats-grid">{[['2024', 'Year Founded'], ['5,000+', 'Nigerians Supported'], ['12+', 'Outreach Movements'], ['100%', 'Community Driven']].map(([number, label]) => <div className="stat-item" key={label}><div className="stat-number">{number}</div><div className="stat-label">{label}</div></div>)}</div></section>
    ...
    <section className="section section-tint"><SectionHeader subtitle="Charity Movements in Action" title="Witness Our Field Work & Impact" description="Take a glance at our recent outreaches, community drives, and video highlights documenting our mission across Nigeria." /><div className="gallery-grid">{mediaItems.slice(0, 3).map((item) => <MediaCard key={item.title} item={item} />)}</div><div className="center-action"><Link href="/media" className="btn btn-outline"><Icon name="video" /> View Full Photo & Video Gallery</Link></div></section>
...
function Media() {
  ...
  const visibleItems = mediaItems.filter((item) => filter === 'all' || item.type === filter || item.category === filter);
  return <><PageBanner background="media_food_outreach.jpg" badge={{ icon: 'photo-video', text: 'Media & Movements Gallery' }} title="Our Charity Movements in Pictures & Videos" description="Explore photos of our field outreaches and YouTube video documentations easing sufferings in Lagos and across Nigeria." /><section className="section"><div className="filter-container">{[['all', 'All Movements'], ['photo', 'Photo Gallery'], ['video', 'YouTube Videos'], ['food', 'Food Drives'], ['medical', 'Medical Outreaches']].map(([key, label]) => <button key={key} className={`filter-btn ${filter === key ? 'active' : ''}`} onClick={() => setFilter(key)}>{label}</button>)}</div><div className="gallery-grid">{visibleItems.map((item) => item.type === 'photo' ? <MediaCard key={item.title} item={item} onOpen={() => setSelected(item)} /> : <VideoCard key={item.title} item={item} />)}</div></section>{selected && <div className="lightbox-modal active" onClick={() => setSelected(null)}><div className="lightbox-content" onClick={(event) => event.stopPropagation()}><button className="lightbox-close" onClick={() => setSelected(null)} aria-label="Close image">&times;</button><img src={image(selected.image)} alt={selected.title} /><div className="lightbox-title"><h4>{selected.title}</h4></div></div></div>}<Cta /></>;
}
function VideoCard({ item }) { return <div className="video-card"><div className="video-embed-container"><iframe src="https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ" title={item.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div><div className="gallery-info"><span className="gallery-tag static-tag"><Icon name="youtube" brand /> YouTube Video</span><h3 className="gallery-title">{item.title}</h3><p className="gallery-desc">{item.description}</p></div></div>; }
